...
export function PortfolioPage() {
  ...
  // Total spend grouped by currency — minor units aren't comparable across them.
  const totalsByCurrency = new Map<string, number>()
  for (const e of entries) {
    if (e.monthlyCostMinor != null && e.currency) {
      totalsByCurrency.set(e.currency, (totalsByCurrency.get(e.currency) ?? 0) + e.monthlyCostMinor)
    }
  }
  const totalLabel = [...totalsByCurrency]
  ...
  return (
    <div className="min-h-full">
      ...
      <main className="mx-auto w-full max-w-5xl px-6 py-10">
        ...
        {portfolio.data && entries.length === 0 && (
          <div className="flex flex-col items-center gap-3 py-24 text-center">
            <p className="text-sm text-ink-muted">No applications yet.</p>
            <p className="max-w-sm text-sm text-ink-subtle">
              Add app and system nodes to your diagrams — they'll roll up here with spend, maturity,
              and how many maps each one appears in.
            </p>
          </div>
        )}
          ...
          <>
            <div className="mb-5 flex flex-wrap items-baseline gap-x-6 gap-y-1 text-sm">
              <span className="text-ink-muted">
                <span className="font-semibold text-ink">{entries.length}</span> applications across
                your maps
              </span>
              {totalLabel && (
                <span className="text-ink-muted">
                  <span className="font-semibold text-ink">{totalLabel}</span> total monthly spend
                </span>
              )}
            </div>
